...
conn.connect((err)=>{
    ...
    query1 = "CREATE TABLE IF NOT EXISTS notification(id INT PRIMARY KEY AUTO_INCREMENT, timestamp TIMESTAMP DEFAULT Now(), courseId INT,studentId INT,message TEXT,status TEXT,FOREIGN KEY(courseId) REFERENCES course(id) ON DELETE CASCADE,FOREIGN KEY(studentId) REFERENCES studentData(id) ON DELETE CASCADE)";
    conn.query(query1,(error,res)=> {
        if(error){
